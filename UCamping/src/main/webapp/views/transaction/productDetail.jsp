		...
		<section>
			<div class="product_box">
				<div class="categories_box">
					<ul>
						<li><a href="${ path }/main.jsp">HOME</a></li>
						<li><a href="${ path }/saleboard/salelist"> > 목록으로 </a></li>
					</ul>
				</div>
				<div class="product_main_area">
							...
							<article class="detail_box_top">
								<section class="detail_box_area">
									<div class="photo_area">
										<div class="main-img">
											<a><img alt="main-img" style="background-color: yellow;"
												class="main_img"
												src="${ path }/resources/upload/saleboard/${ SaleBoard.fileSystemName }"></a>
										</div>
									...
									<div class="item_info">
										...
										<div class="item_count_sales_box">
											<div class="item_info_count">
												<ul>
													<li><img
														src="${ path }/resources/images/wish_heart.png"
														width="15px" height="15px">0</li>
											...
											<section class="item_user_info">
												<div class="profile_img">
													<a href=""> <img class="profile"
														src="${ path }/resources/images/pro_photo.jpg"
														alt="users_profile_img">
													</a>
												</div>
												<div class="nick">
													<a href=""> ${ saleboard.writerId } </a>
												</div>
												<div class="users_item_count">
													<a href=""> <span class="users_item_count_title">상품</span>
														<span class="users_item_count_number">10</span>
													</a>
												</div>
												<div class="detail_profile_review">
													<a href=""> <span class="review_tag">거래후기</span> <span
														class="review_count_rating"> (0) </span> <span
														class="review_count_rating_img"> <img
															src="${ path }/resources/images/pro_image/review_star_1.png"
															alt="blank_star1"> <img
															src="${ path }/resources/images/pro_image/review_star_1.png"
															alt="blank_star2"> <img
															src="${ path }/resources/images/pro_image/review_star_1.png"
					...
					<table id="tbl-comment">
						<c:forEach var="reply" items="${ SaleBoard.replies }">
							<c:set var="i" value="${i+1}" />
							<tr class="level1">
								<td><sub class="comment-writer"><c:out
											value="${ reply.writerId }" /></sub> <sub class="comment-date"><fmt:formatDate
											type="date" value="${ reply.createDate }" /></sub> <br> <c:out
										value="${ reply.content }" /></td>
								<td><c:if
										test="${ ! empty loginMember && loginMember.id == reply.writerId }">
										<button type="button" class="btn-delete" value="${ reply.no }">삭제</button>
									</c:if></td>
							</tr>
						</c:forEach>
					</table>
					...
					<section class="member_comment">
						<div class="member_comment_count">
							<section class="comment_count">
								<span class="comment_count_all">댓글</span> <span>0</span>
							</section>
							<section class="input_comment">
								<form id="reply-form"
									action="${ pageContext.request.contextPath }/productDetail/reply"
									method="post">
									<div class="cmt_in_box">
										<input type="hidden" name="boardNo" value="${ SaleBoard.no }">
										<input type="hidden" name="writer"
											value="${ SaleBoard.writerId }">
										<textarea name="content" placeholder="댓글을 입력해주세요."
											class="input_txt"></textarea>
									</div>
									<div class="cmt_btn_box">
										<button type="submit" class="cmt_send_btn" id="btn-insert">댓글등록</button>
...
<script>
            $(document).ready(()=>{
                $(".btn-delete").on("click", ()=>{
                	 let no = $(event.target).val();

                    if(confirm("정말로 댓글을 삭제하겠습니까?")){
                        location.replace("${ pageContext.request.contextPath }/productDetail/delete?no=${ SaleBoard.no }&replyNo="+no);
                    }
                }) 
            });
        </script>
